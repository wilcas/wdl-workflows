...
workflow BamToUnmappedBams {
  ...
  call OutputFile{
    input:
  	  bam_base = bam_basename,
      unmapped_bams = SortSam.sorted_bam,
      disk_size = ceil(input_size * 3) + additional_disk_size,
      docker = gatk_docker,
      machine_mem_gb = 2,
  ...
  output {
    ...
    File unmapped_bams_list = OutputFile.unmapped_bams_list
  }
}
...
task OutputFile {
  ...
  command <<<
  set -exo pipefail
  for x in ~{sep=' ' unmapped_bams}; do
     echo "${x/\/cromwell_root\//gs:\/\/}" >> ~{bam_base}.unmapped_bams_list
  done;
  >>>
  runtime {
    docker: docker
  ...
  output{
    File unmapped_bams_list = "~{bam_base}.unmapped_bams_list"
  }
